# -*- coding: utf-8 -*-
"""
Created on Wed Dec  4 20:00:20 2019
...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  4 15:18:51 2019
...
建立神經網路，並加入 L1_L2
"""
def build_mlp(input_shape, output_units=10, num_neurons=[512, 256, 128], l1l2_ratio=1e-4):
    input_layer = keras.layers.Input(input_shape)
...
for regulizer_ratio in L1L2_EXP:
    keras.backend.clear_session() # 把舊的 Graph 清掉
    print("Experiment with Regulizer = %.6f" % (regulizer_ratio))
    model = build_mlp(input_shape=x_train.shape[1:], l1l2_ratio=regulizer_ratio)
    model.summary()
    optimizer = keras.optimizers.SGD(lr=LEARNING_RATE, nesterov=True, momentum=MOMENTUM)
    model.compile(loss="categorical_crossentropy", metrics=["accuracy"], optimizer=optimizer)
